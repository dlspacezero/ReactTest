# 自我介绍
```
面试官你好，我叫刘婳祎来自内蒙，目前四年工作经验，从事前端开发三年。那么我以时间线索阐述一下我的个人经历吧。我是17年本科毕业，大四的时候学校鼓励提前就业，我当时是直接去一家内蒙的互联网公司做了交互设计，本身有些代码基础，在工作的过程中和前端接触也比较多，对代码更是产生了浓厚的兴趣，因为设计还是较为感性的，我更喜欢代码这种理性思维。拿到毕业证后，在同一家公司转了前端开发。工作中与设计的工作对接也是很流畅友好的。毕竟在内蒙，互联网行业还是比较滞后，所以18年10月份的时候我来到杭州,在一家阿里外包做前端开发。先后服务过蚂蚁金服芝麻信用团队以及相互宝团队。参与过天蚕平台、相互宝审核后台等大型项目开发。
```
...
5.cancel requset逻辑需要额外做验证
	用户访问页面会先调用setup的dispatch({ type: 'query' })，这个query会去请求权限，然后进入到request里面，此时如果有页面切换，那么这个query权限的请求就肯定会被cancel，（cancel逻辑代码在reqest.js里）
	解决方案：
	在request.js里面做判断，如果是查询权限的不应该做cancel处理

6.工作中遇到的困难 
	1）IE6中高度不对问题 
		在div中给定了高度为1px,其它浏览器显示正常，可是ie6中显示的高度就不对了，这时我给样式表中加了个font-size:0px;line-height:0px;就好了 
...
# 性能优化
```
1、减少http请求次数，因为80%的响应时间花在下载网页内容，所以减少http请求次数是缩短响应时间的关键
2、减少DNS查询次数，DNS查询也消耗响应时间，尽量将外部域的对象下载到本地服务器上等等 
3、减少DOM元素数量，网页中元素过多对网页的加载和脚本的执行都是沉重的负担，因此要减少DOM元素的数量。
4、尽量不要在页面的主体布局中使用table，table要等其中的内容完全下载之后才会显示出来，显示比div+CSS布局慢。 
5、对图片进行处理，比如说精灵图、base64格式、或者使用字体图标代替图片
6、减小资源的大小，对html、css、图片等进行压缩
7、优化资源加载的位置和时机，比如js放在body底部
8、body中不要写style标签和script标签，模块按需加载根据路由来加载当前页面需要的业务模块、还有懒加载
9、减少重绘与回流
```
...
```
ajax是一种交互式网页开发技术，通过在后端与服务器进行少量数据交换，可以使网页实现异步(局部)更新
ajax最大的特点就是可以把服务器负担的工作转移到客户端，利用客户端的闲置的资源进行处理，减轻服务器和带宽的负担，节约空间和成本；可以调用xml、json、php等外部数据
ajax的核心是XMLHttpRequest:发送请求到服务器并获取返回结果
ajax四部曲是
1.创建ajax对象(存在兼容)
...
4.设置回调函数--监听就绪状态码 
  	...
  	- ajax.readyState==4 代表ajax的工作状态完成，共5个值
...
# 事件轮询
```
是指浏览器的一种解决JavaScript单线程运行时不阻塞的一种机制，单线程的是所有任务都在主线程上完成,任务太多的时候,页面卡死,eventLoop可以解决单线程阻塞问题,程序中会有两个线程,一个主线程,一个eventLoop线程,负责主线程和其他进程之间的通信,遇到I/O的时候,主线程会让eventLoop线程通知对应的程序,主线程的任务会继续往后执行,等I/O程序执行完了,eventLoop线程会把结果返回给主线程,主线程利用回调函数调用结果,完成任务
（1）所有同步任务都在主线程上执行，形成一个执行栈（execution context stack）。
...
```
this指向：
this是js的一个关键字，它代表函数运行时自动生成的一个内部对象，只能在函数内部使用。随着函数使用场合的不同，this的值会发生变化。但是有一个总的原则，就是谁调用指向谁
作为普通函数调用，如函数表达式、自执行函数、定时器内部函数、回调函数都指向window；作为对象的方法，如事件处理函数，都指向当前对象；作为构造函数，就指向了实例对象

改变this指向：
改变this指向第一种方法是可以通过函数自带的call、apply、bind；他们的第一个参数都是新的指向；call和apply的区别是，call从第二个参数开始就是函数的参数，而apply第二个参数是一个数组，里面也是参数(一个[]的区别)call和bind参数一样，区别是call返回一个函数体，需要再次调用(一个()的区别)；第二个方法是通过储存变量的方式，比如说that=this；第三个可以通过new关键，构建一个实例对象
```
...
观察者模式（发布订阅模式）
	定义对象间的一种一对多的依赖关系，当一个对象的形状发生变化时，所有的依赖于它的对象将受到通知。
...
# js的数据类型
```
Js我知道的有7种数据类型,包括6种基本数据类型和1种引用类型
基本数据类型有：undefined、null、boolean、number、string、还有es6新增的symbol，表示独一无二的值
引用数据类型是object，其本质是键值对的组合，包含function、array、data

基本数据类型直接存储在栈中，占据空间小、大小固定，会被被频繁使用数据，所以放入栈中存储。
引用数据类型同时存储在栈和堆中，占据空间大、大小不固定。引用数据类型在栈中存储了指针，该指针指向堆中该实体的起始地址。当解释器寻找引用值时，会首先检索其在栈中的地址，取得地址后从堆中获得实体。
```
...
```
事件委托本质上是利用了事件冒泡的机制，父节点可以通过事件对象获取到目标节点，所以把子节点的监听函数定义在父节点上，由父节点的监听函数统一处理多个子元素的事件。
使用事件委托可以减少DOM查找次数，减少事件注册，给未来元素添加事件，节约内存，提升性能但是如果所有事件都用事件代理，可能会出现事件误判。把本不该被触发的事件被绑定上了事件
```
...
```
typeof：不准确，检测null和数组的时候都是对象
instanceof: 不准确，检测数组时，数组既属于数组也属于对象
constructor：检测null和undefined的constructor都会报错
toString(): 最准确，Object.prototype.toString.call('啥').slice(8, -1)
typeof和instanceof的区别：typeof是判断变量是什么基本类型的，而instanceof是判断对象到底是什么类型的
```
...
原型链：
javascript是面向对象的，每一个实例对象都有一个__proto__属性，这个指针指向构造函数的原型，也就是构造函数的prototype，保存了这个构造函数的公有属性和方法。在prototype里又有一个指针，叫做constructor，它指向的原型所在的构造函数。当一个对象查找属性的时候，先在自身查找，如果自身没有，就沿着原型链一级一级向上查找,直到查找到Object.prototype.__proto__为null,这样也就形成了原型链。
简单来说，实例对象与原型之间的连接，叫做原型链。
...
```
箭头函数是普通函数的简写，语法简洁
箭头函数是匿名函数，不能作为构造函数使用，所以不能new
箭头函数没有原型属性
普通函数的this谁调用指向谁，箭头函数的this指向定义时所在作用域的this，不可以改变
```
...
# css3如何绘制三角形
```
先新建一个容器，设置样式width和height都为0,其他两个边用全透明属性transparent，另一个边正常设置就可以了
```
...
常见的兼容问题
一、event对象下的兼容
1.事件对象event，标准浏览器(chrome firefox）ev是事件的第一个参数，普通浏览器(IE8及以下）ev是window的子属性
2.添加事件监听，标准浏览器是target.addEventListener("事件类型", 函数, 是否捕获(布尔值))，普通浏览器是target.attachEvent("事件类型",函数)
3.阻止默认行为，标准浏览器ev.preventDefault()，普通浏览器ev.returnValue = false
4.取消事件监听，标准浏览器target.removeEventListener()，普通浏览器target.detachEvent()
5.取消冒泡，标准浏览器ev.stopPropagation()，普通浏览器ev.cancelBubble=true(ie+chrome)
6.键盘事件，标准浏览器ev.which，普通浏览器ev.keyCode
7.目标，标准浏览器ev.target，普通浏览器ev.srcElement
8.滚动条，标准浏览器document.documentElement.scrollTop(scrollLeft)，普通浏览器document.body.scrollTop(scrollLeft)

二、CSS属性兼容
1.透明度，标准浏览器opacity:1;普通浏览器filer:alpha(opacity=100);

三、阿贾克斯
创建ajax请求，标准浏览器xhr = new XMLHttpRequest()，普通浏览器xhr=new ActiveXObject('Microsoft.XMLHTTP')

处理的方式一般使用：
1.if语句
2.或运算符 - ||
3.三目运算符
4.容错处理 - try...catch…finally
...
```
1.Node不是一个后端语言，但是它可以做类似后端语言的功能，一种javascript的运行环境，能够使得javascript脱离浏览器运行。
2.Node是使用谷歌V8引擎
4.Node具有非阻塞I/O 特点
...
如何避免回调地狱？
模块化，将回调函数分割为独立的函数；使用Promises
类Unix系统中不可以尝试去监听80端口，因为这需要超级用户权限。直接监听大于1024的端口。
```
...
```
1.设置line-height和父元素一样高垂直居中，text-align水平居中
2.父元素设置relative,要居中的元素设置asolute,margin-left/top都设置自身宽度高度的一半，left和top设置50%
3.给父元素加relative,要居中的元素加absolute,margin-left/top/right/bottom都为0，再设置margin-auto
4.弹性盒，父元素设置display：flex，子元素设置margin-auto
5.用弹性盒里的两个属性just-contents和align-items都居中，有局限性，只能有一个元素使用    
6.定位+transfrom:父元素加relative,要剧中的元素absolute,top/left设置50%，transform：translate -50%;
```
...
```
（1）通过媒体查询的方式即CSS3的meida queries，主要是通过查询设备的宽度来执行不同的 css 代码，最终达到界面的配置。
	优点：
...
（2）通过flex弹性布局
	它的viewport是固定的，在meta标签里写东西<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">，把高度定死宽度进行自适应。随着屏幕宽度变化，页面也会跟着变化，在哪个宽度需要调整的时候使用响应式布局调调就行，这样就实现了适配。

（3）以淘宝首页为代表的 rem+viewport缩放
	根据屏幕宽度设定rem值，需要适配的元素都使用rem为单位，不需要适配的元素还是使用px为单位。（1em = 16px）
	实现原理：
...
（4）rem 方式
	viewport也是固定的，在meta标签里写东西。控制rem基准值
```
...
```
es5可以通过原型链进行继承，就是通过原型让一个引用类型继承另一个引用类型的属性和方法，其实就是让原型对象等于另一个类型的实例
也可以通过构造函数进行继承，就是在子类中调用父类，通过使用apply()和call()方法可以在将来新创建的对象上执行构造函数
还可以通过混合模式，也就是构造函数+原型进行继承的,使用原型链实现对原型属性和方法的继承，用借用构造函数模式实现对实例属性的继承
es6新增了class,通过extends继承，子类必须在constructor方法中调用super方法，因为子类没有自己的this对象，而是继承父类的this对象，然后对其进行加工，如果不调用super方法，子类就得不到this对象
```
...
# h5新增了哪些内容
```
语义化标签，如header，footer，nav，section，article，aside等
媒体标签，如audio，video
功能性标签，如svg，canvas
智能表单，如一些input的type类型，和新的input属性
本地存储，如localstorage和sessionstorage
还有一些api，如fetch(浏览器自带的数据请求方式)和requestAnimationFrame（动画定时器）等等
```
# display有哪些属性值
```
block: 块状元素
